...
set clock_period 35000
set spi_period 70000
set input_clock_delay [expr $clock_period * 0.2]
set output_clock_delay [expr $clock_period * 0.2]
set input_spi_delay [expr $spi_period * 0.2]
set output_spi_delay [expr $spi_period * 0.2]
...
set_input_delay -clock $clock_name -max $input_clock_delay [get_ports {rst_ni fetch_enable_i en_ifetch_i}]
set_input_delay -clock $clock_name -min $input_clock_delay [get_ports {rst_ni fetch_enable_i en_ifetch_i}]
set_output_delay -clock $clock_name -max $output_clock_delay [get_ports {gpio*}]
set_output_delay -clock $clock_name -min $output_clock_delay [get_ports {gpio*}]
...
set_input_delay -clock $spi_name -max $input_spi_delay [get_ports {spi_cs spi_sdi*}]
set_input_delay -clock $spi_name -min $input_spi_delay [get_ports {spi_cs spi_sdi*}]
set_output_delay -clock $spi_name -max $output_spi_delay [get_ports {spi_mode* spi_sdo*}]
set_output_delay -clock $spi_name -min $output_spi_delay [get_ports {spi_mode* spi_sdo*}]
